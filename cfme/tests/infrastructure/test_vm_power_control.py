...
from utils.randomness import generate_random_int, generate_random_string
...
def pytest_generate_tests(metafunc):
    ...
    if 'random_pwr_ctl_vm' in metafunc.fixturenames:
        global random_vm_test
        if len(random_vm_test) == 0:
            single_index = generate_random_int(max=(len(idlist) - 1))
            ...
            argnames = random_vm_test[0]
            new_argvalues = random_vm_test[1]
            new_idlist = random_vm_test[2]
    else:
        new_idlist = idlist
...
@pytest.fixture(scope="class")
def test_vm(request, provider_crud, vm_name, provider_init):
    '''Fixture to provision appliance to the provider being tested if necessary'''

    vm = Vm(vm_name, provider_crud)
    if not provider_crud.get_mgmt_system().does_vm_exist(vm_name):
    ...
    def finalize():
        ...
        if provider_crud.get_mgmt_system().is_vm_suspended(vm_name):
            logger.info("Powering up VM %s to shut it down correctly on %s." %
                (vm_name, provider_crud.key))
            provider_crud.get_mgmt_system().start_vm(vm_name)
        if provider_crud.get_mgmt_system().is_vm_running(vm_name):
            logger.info("Powering off VM %s on %s" % (vm_name, provider_crud.key))
            provider_crud.get_mgmt_system().stop_vm(vm_name)
        if provider_crud.get_mgmt_system().does_vm_exist(vm_name):
            logger.info("Deleting VM %s in %s" % (vm_name, provider_crud.key))
            provider_crud.get_mgmt_system().delete_vm(vm_name)
